...
namespace OnlineTest.Services.Services
{
    public class AnswerService : IAnswerService
    {
        ...
        #region Methods
        public ResponseDTO GetAnswersDTO()
        {
        ...
        public ResponseDTO AddAnswersDTO(AddAnswerDTO answer)
        {
            var response = new ResponseDTO();
            try
            {
        ...
        public ResponseDTO UpdateAnswersDTO(UpdateAnswerDTO answer)
        {
            var response = new ResponseDTO();
            try
            {
                var answerById = _answerRepository.GetAnswersById(answer.Id);
                if (answerById == null)
                {
                    response.Status = 404;
        ...
        public ResponseDTO GetAnswersById(int id)
        {

            var response = new ResponseDTO();
            try
            {
                var data = _mapper.Map<GetAnswerDTO>(_answerRepository.GetAnswersById(id));
                response.Status = 200;
                response.Message = "Ok";
                response.Data = data;
            }
        ...
        public ResponseDTO DeleteAnswer(int id)
        {
            ...
            try
            {
                var answerById = _answerRepository.GetAnswersById(id);
                if (answerById == null)
                {
                    response.Status = 404;
                    response.Message = "Not Found";
